...
<div>
  ...
  <!--轮播图及内容-->
  <div class="goodsMain" v-show="main">
    <div style="position:relative; background:#f9f8fd" class="row">
      <div class="detail-img" v-if="productPhoto">
        <wv-swipe class="demo-swipe" :height="300" :auto="4000" :continuous="continuous">
          <wv-swipe-item class="demo-swipe-item" v-for="img in productPhoto" :key="img.id">
    ...
    <div class="tab-content">
      <div class="goodsBrief" v-if="profile" v-html="profile.profile"></div>
      <!-- <div class="goodsTxt">
...
export default {
  asyncData (store, route) {
    // 去加载轮播和楼层数据
    return store.dispatch('fetchProduct', route.query.gid).then(() => loading && loading.hide())
  },
  data() {
    return {
  ...
  activated() {
    ...
    // 去获取商品详情数据
    this.$store.dispatch('fetchProductProfile', this.pid)
    // 隐藏底部主菜单
  ...
  computed: {
    ...mapState({
      main: state => state.shop.product && state.shop.product.main,
      productPhoto: state => state.shop.product && state.shop.product.photo
    }),
    ...mapGetters({
  ...
  methods: {
    setPageConfig() {
      if (this.main) {
        // 设置网页标题
        this.$parent.setTitle(this.main.productName)
        // 微信分享初始化->(title, desc, imgUrl, link)
        let desc = '【足力购】帅哥美女们，快来足力购逛逛，捧个场吧！'
        this.initWechatShare(
          this.main.productName + '-足力购商城',
          desc,
          this.img_domain + (this.productPhoto.length > 0 ? this.productPhoto[0].resource : ''),
          window.location.href
        )
        // 如果loading还在则取消显示
        loading && loading.hide()
      } else {
        $.toast('未能获取到商品', 'forbidden')
      }
    },
    pageHtml() {
